...
import { getAllTeacherHomeworkAPIcall } from '../../../Redux/Actions/schoolTeacherAction';
...
const Index = () => {
    ...
    const homeworkData = useSelector((state: any) => state.schoolTeacherReducer.teacherHomework);
    ...
    useEffect(() => {
        if (findRoutes.hasOwnProperty('state')) {
            if ((findRoutes as any)?.state?.hasOwnProperty('teacherData')) {
                const infodata = (findRoutes as any).state.teacherData
    ...
    useEffect(() => {
        if (homeworkData?.result2) {
            const data = homeworkData.result2.map((item: any, i: number) => {
                return {
                    rowNumber: i,
            ...
            const result = homeworkData.result2.map((item: any) => {
